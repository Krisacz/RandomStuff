...
namespace Test
{
    class Program
    {
        ...
        private static string Process(string inputStr, bool additionalDebugOutput = false)
        {
            ...
            //Last character is ">"
            if (inputStr.Length > 0 && inputStr[inputStr.Length - 1] == '>') return "ERROR: Incorrect input string. Missing dependency job. Input string ends with \">\".";
            #endregion
            ...
            for(var i1 = 0; i1 < tokens.Count; i1++)
            {
                ...
                if (string.Equals(job1, dependencyJob1)) return $"ERROR: Job can not depends on itself. [{job1} > {dependencyJob1}]";
                #endregion
            ...
            //We are processinf all tokens untill all tokens are in correct place
            var correct = false;
            while (!correct)
            {
